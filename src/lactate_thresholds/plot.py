...
def lactate_intensity_plot(x: LactateThresholdResults):
    ...
    ## add the LTPs
    plt.axvline(x.ltp1.intensity, color="red", linestyle="--")
    plt.axvline(x.ltp2.intensity, color="red", linestyle="--")
    ## add info next to the LTP lines about hearrate, lactate and intensity
   
    intensity_range = x.interpolated_data['intensity'].max() - x.interpolated_data['intensity'].min()
    ...
    sns.scatterplot(x="intensity", y="lactate", data=x.clean_data[x.clean_data["intensity"] > 0], color="black")